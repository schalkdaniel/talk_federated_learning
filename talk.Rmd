...
-   Each dataset gives an empirical risk $\mathcal{R}_\mathsf{emp}(\mathcal{D}_k, \theta)$
...
Instead of making one huge update on the host side, we conduct the update on the client side and average the updates:
$$
...
-   **Derivative of the Loss Function** $$\frac{\delta}{\delta\theta}L(y,f\left(x,\theta)\right) = x \left(y - f(x,\theta)\right)$$
...
With
$$
r^{[m](i)} = - \left[\frac{\delta}{\delta f} L\left((y^{(i)},f(x^{(i)})\right)\right]_{f = f^{[m]}}
$$
