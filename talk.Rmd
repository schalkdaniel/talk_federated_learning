...
-   Now a statistician wants to analyze the data and predict whether a patient is diseased or not on a global machine (the host)
-   **But:** Most statistical or machine learning approaches require **one** database for modeling

\begin{itemize}
  \item[$\Rightarrow$] We want to learn one model on decentralized datasets distributed over multiple sources.
\end{itemize}

\includegraphics[width=4cm,page=6]{images/federated_learning.pdf}
...
| **Risk**             | $\mathcal{R}(\mathcal{D}, \hat{\theta}) = \mathcal{R}_\mathsf{emp}(\mathcal{D}, \hat{\theta}) + \lambda C(\theta)$ | 
...
\begin{align*}
\hat{\theta}^{[t+1]} &= \hat{\theta}^{[t]} - \eta\nabla_\theta \mathcal{R}(\mathcal{D}, \hat{\theta}^{[t]}) \\
...
-   Each client holds a non-distributable local dataset $\mathcal{D}_k$, $k \in \{1, \dots, K\}$ with $n_k$ observations
-   Each dataset gives a risk $\mathcal{R}(\mathcal{D}_k, \theta)$ $\rightarrow$ $g_k(\theta) = \nabla_\theta\mathcal{R}(\mathcal{D}_k, \theta)$
...
$$
\hat{\theta}^{[t]} = \mathsf{avg}(\gamma_1, \dots, \gamma_K, \alpha) = \sum\limits_{k = 1}^K \frac{n_k}{n} \gamma_{k}
$$
...
\begin{algorithm}[H]
  ...
  \While{Stop criteria is not reached}{
    ...
    \tcc{Host conduct Federated Averaging step:}
    $\hat{\theta}^{[t]} = \mathsf{avg}(\gamma_1, \dots, \gamma_K, \alpha)$\;
    Check if stop criteria is reached, e.g. $\|\hat{\theta}^{[t]} - \hat{\theta}^{[t-1]}\|_2 < \varepsilon$\;
    Increment $t \leftarrow t + 1$ \;
  }
...
-   We assume that there are $4$ hospitals, each with a local non-distributable dataset
-   The task is to predict whether a patient is diseased or not
...
-   Sequential fitting of an additive model by refitting base-learner on the previous error
...
\begin{itemize}
\item[$\Rightarrow$] To apply Federated Learning on Boosting we have to aggregate functions representing the functional gradient
\end{itemize}
...
-   Inherent evaluation on local datasets during the fitting process

-   Inherent assessment of the clients data quality  

-   Varying iterations on client side or use another stop criteria there
